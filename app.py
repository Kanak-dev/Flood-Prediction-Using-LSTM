...
def get_predicts():
    try:
        ...
        # Geolocation lookup (OpenCage in this branch)
        # Note: OpenCage expects the API key parameter named 'key' and returns 'results' with 'geometry'.
        URL = "https://api.opencagedata.com/geocode/v1/json"
        # Prefer environment variable so you can replace the key without editing source
        api_key = os.environ.get('OPENCAGE_KEY', '44c6f14eec534f3e9b2f753ec7e65d41')
        if not api_key:
            print("API key for OpenCage is not set")
            return render_template('predicts.html', cities=cities, cityname="Server configuration error: missing geocode API key.")
        ...
        try:
            ...
            print("Network error when calling geocode API:", re)
            return render_template('predicts.html', cities=cities, cityname="Network error fetching location data.")

        print("API Response Code:", r.status_code)
        # Helpful diagnostic: print response body when non-200 so you can see provider's message
        ...
        # OpenCage returns 'results' list containing geometry with lat/lng
